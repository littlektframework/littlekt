...
final class MemoryManage {
    ...
    static class DebugAllocator implements MemoryAllocator {
        ...
        @Override
        public long getMalloc() {
            return allocator.getMalloc();
        }

        @Override
        public long getCalloc() {
            return allocator.getCalloc();
        }

        @Override
        public long getRealloc() {
            return allocator.getRealloc();
        }

        @Override
        public long getFree() {
            return allocator.getFree();
        }

        @Override
        public long getAlignedAlloc() {
            return allocator.getAlignedAlloc();
        }

        @Override
        public long getAlignedFree() {
            return allocator.getAlignedFree();
        }
                    ...
                    if (groupByThread) {
                        ...
                        for (Allocation allocation : ALLOCATIONS.values()) {
                            Map<StackTraceElement, AtomicLong> mapMethod = mapThreadMethod.computeIfAbsent(allocation.threadId, new Function<Long, Map<StackTraceElement, AtomicLong>>() {
                                @Override
                                public Map<StackTraceElement, AtomicLong> apply(Long k) {
                                    return new HashMap<>();
                                }
                            });
                    ...
                    if (groupByThread) {
                        ...
                        for (Allocation allocation : ALLOCATIONS.values()) {
                            Map<Allocation, AtomicLong> mapStackTrace = mapThreadStackTrace.computeIfAbsent(allocation.threadId, new Function<Long, Map<Allocation, AtomicLong>>() {
                                @Override
                                public Map<Allocation, AtomicLong> apply(Long k) {
                                    return new HashMap<>();
                                }
                            });
