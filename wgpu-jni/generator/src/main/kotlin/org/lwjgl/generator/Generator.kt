...
enum class Binding(val key: String, val packageName: String, val arrayOverloads: Boolean = true) {
...
fun String.dependsOn(vararg bindings: Binding): String? =
    if (bindings.any { it.enabled }) this else null

fun main(args: Array<String>) {
    if (args.size < 2)
        throw IllegalArgumentException(
            "The code Generator requires 2 paths as arguments: a) the template source path and b) the generation target path"
        )
    ...
    generate(args[0], args[1]) {
        ...
        // For a package passed to the generate function, we  search for <package>.templates.* class
        // files and run any public static methods that return
        // NativeClass objects.
        ...
        // Example: org.lwjgl.opengl -> org.lwjgl.opengl.OpenglPackage (the first letter is
        // capitalized)
        ...
        val bindingsModular = Binding.entries.asSequence()
...
private fun generate(srcPath: String, trgPath: String, generate: Generator.() -> Unit) {
    Generator(srcPath, trgPath).generate()
}

class Generator(val srcPath: String, val trgPath: String) {
    ...
    companion object {
        ...
        fun register(struct: Struct): Struct {
            if (Binding.isEnabled(struct.packageName)) structs.add(struct)
            return struct
        }

        /** Registers a callback function. */
        fun register(callback: CallbackFunction) {
            if (Binding.isEnabled(callback.packageName)) callbacks.add(callback)
        }

        /** Registers a custom class. */
        fun <T : GeneratorTarget> register(customClass: T): T {
            if (Binding.isEnabled(customClass.packageName)) customClasses.add(customClass)
            return customClass
        }

        fun registerLibraryInit(
            packageName: String,
            className: String,
            libraryName: String,
            setupAllocator: Boolean = false
        ) {
            if (!Binding.isEnabled(packageName)) return

            Generator.register(
                object : GeneratorTargetNative(packageName, className) {
                    init {
                        access = Access.INTERNAL
                        documentation = "Initializes the $libraryName shared library."
                        javaImport("org.lwjgl.system.*")
                        if (setupAllocator) javaImport("static org.lwjgl.system.MemoryUtil.*")
                        nativeDirective(
...
#include "lwjgl_malloc.h""""
    ...
    private val GENERATOR_LAST_MODIFIED =
        sequenceOf(Paths.get("wgpu-jni/generator/src/main/kotlin").lastModified()).fold(0L, ::max)
    ...
    internal fun generate(packageName: String, binding: Binding? = null) {
        ...
        apply(packagePath, packageName) {
            this.filter { methodFilter(it, Void.TYPE) }.forEach { it.invoke(null) }
        }
        ...
        apply("$packagePath/templates", "$packageName.templates") {
            this.filterTo(templates) { methodFilter(it, NativeClass::class.java) }
        }
        ...
        for (template in templates) {
            ...
            if (nativeClass.packageName != packageName)
                throw IllegalStateException(
                    "NativeClass ${nativeClass.className} has invalid package [${nativeClass.packageName}]. Should be: [$packageName]"
                )
    ...
    private fun generate(nativeClass: NativeClass, packageLastModified: Long) {
        ...
        val lmt =
            max(nativeClass.getLastModified("$srcPath/$packagePath/templates"), packageLastModified)
        ...
        generateOutput(nativeClass, outputJava, lmt) { it.generateJava() }

        if (!nativeClass.skipNative) {
            generateNative(nativeClass) { generateOutput(nativeClass, it) { it.generateNative() } }
        } else nativeClass.nativeDirectivesWarning()
    }
        ...
        targets.forEach {
            try {
                ...
                throw RuntimeException(
                    "Uncaught exception while generating $typeName: ${it.packageName}.${it.className}",
                    e
                )
            }
        }
    ...
    internal fun generate(target: GeneratorTarget) {
        ...
        val lmt =
            if (target.sourceFile == null) null
            else
                max(
        ...
        generateOutput(target, outputJava, lmt) { it.generateJava() }

        if (target is GeneratorTargetNative && !target.skipNative) {
            generateNative(target) { generateOutput(target, it) { it.generateNative() } }
        }
    }
...
internal val Path.lastModified
    get() = if (Files.isRegularFile(this)) Files.getLastModifiedTime(this).toMillis() else 0L
...
internal fun Path.lastModified(
    ...
    glob: String? = null,
    matcher: PathMatcher =
        if (glob == null) KOTLIN_PATH_MATCHER
        else FileSystems.getDefault().getPathMatcher("glob:$glob")
): Long {
...
private fun ensurePath(path: Path) {
    val parent =
        path.parent ?: throw IllegalArgumentException("The given path has no parent directory.")
...
private fun readFile(file: Path) =
...
private fun <T> generateOutput(
    ...
    if (Files.isRegularFile(file)) {
        ...
        LWJGLWriter(OutputStreamWriter(baos, Charsets.UTF_8)).use { target.generate(it) }
        ...
        fun somethingChanged(before: ByteBuffer, after: ByteArray): Boolean {
            if (before.remaining() != after.size) return true
        ...
        LWJGLWriter(Files.newBufferedWriter(file, Charsets.UTF_8)).use { target.generate(it) }
    }
}

/** Returns true if the array was empty. */
internal inline fun <T> Array<out T>.forEachWithMore(apply: (T, Boolean) -> Unit): Boolean {
    for (i in 0..this.lastIndex) apply(this[i], 0 < i)
    return this.isEmpty()
}

/** Returns true if the collection was empty. */
internal fun <T> Collection<T>.forEachWithMore(
    moreOverride: Boolean = false,
    apply: (T, Boolean) -> Unit
): Boolean = this.asSequence().forEachWithMore(moreOverride, apply)

/** Returns true if the sequence was empty. */
internal fun <T> Sequence<T>.forEachWithMore(
    moreOverride: Boolean = false,
    apply: (T, Boolean) -> Unit
): Boolean {
