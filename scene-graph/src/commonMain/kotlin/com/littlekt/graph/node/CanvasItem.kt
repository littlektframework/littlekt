...
abstract class CanvasItem : Node() {
    ...
    /**
     * List of 'preRender' callbacks called when [onPreRender] is called. Add any additional
    ...
    /**
     * List of 'render' callbacks called when [propagateInternalRender] is called. Add any
    ...
    /**
     * List of 'postRender' callbacks called after [postRender] is called. Add any additional
     * callbacks directly to this list. The main use is to add callbacks directly to nodes inline
     * when building a [SceneGraph] vs having to extend a class directly.
     *
     * ```
    ...
    /**
     * List of 'debugRender' callbacks called when [debugRender] is called. Add any additional
     * callbacks directly to this list. The main use is to add callbacks directly to nodes inline
     * when building a [SceneGraph] vs having to extend a class directly.
     *
     * ```
    ...
    /**
     * List of 'onVisible' callbacks called when [onVisible] is called. Add any additional callbacks
     ...
     * ```
     */
    @JsName("onVisibleSignal") val onVisible: Signal = signal()

    /**
     * List of 'onInvisible' callbacks called when [onInvisible] is called. Add any additional
     * callbacks directly to this list. The main use is to add callbacks directly to nodes inline
     * when building a [SceneGraph] vs having to extend a class directly.
     *
     * ```
     ...
     * ```
     */
    @JsName("onInvisibleSignal") val onInvisible: Signal = signal()

    /**
     * Shows/hides the node if it is renderable. This item will receive no [render] calls and is
     * hidden but will continue to receive [update] calls.
     */
    var visible: Boolean
        get() = _visible
        set(value) {
    ...
    /**
     * The position of the [CanvasItem] in global space. If you want to set the [x,y] properties of
     * this [Vec2f] then use the [globalX] and [globalY] properties of this [CanvasItem]
     */
    ...
    /**
     * The position of the [CanvasItem] relative to the parent transform. If the [CanvasItem] has no
     * parent or if the parent node is NOT a [CanvasItem], then it is the same a [globalPosition].
     * If you want to set the [x,y] properties of this [Vec2f] then use the [x] and [y] properties
     * of this [CanvasItem]
     */
    var position: Vec2f
        get() {
    ...
    /**
     * The x-position of the [CanvasItem] relative to the parent transform.
     *
     * @see [position]
     */
    var x: Float
    ...
    /**
     * The y-position of the [CanvasItem] relative to the parent transform.
     *
     * @see [position]
     */
    var y: Float
    ...
    /**
     * The rotation of the [CanvasItem] relative to the parent transform's rotation. If the
     * [CanvasItem] has no parent or if the parent node is NOT a [CanvasItem], then it is the same a
     * [globalRotation]
     */
    var rotation: Angle
        get() {
    ...
    /**
     * The global scale of the [CanvasItem]. If you want to set the [x,y] properties of this [Vec2f]
     * then use the [globalScaleX] and [globalScaleY] properties of this [CanvasItem].
     */
    ...
    /**
     * The scale of the [CanvasItem] relative to the parent transform's scales. If the [CanvasItem]
     * has no parent or if the parent node is NOT a [CanvasItem], then it is the same a
     * [globalScale]. If you want to set the [x,y] properties of this [Vec2f] then use the [scaleX]
     * and [scaleY] properties of this [CanvasItem].
     */
    var scale: Vec2f
        get() {
            updateTransform()
    ...
    /** The inverse matrix of [localToGlobalTransform]. */
    ...
    /**
     * Internal debug rendering that needs to be done on the node that shouldn't be overridden.
     * Calls [propagateInternalDebugRender] method.
     */
    ...
    /**
     * Internal pre rendering that needs to be done on the node that shouldn't be overridden. Calls
     * [propagatePreRender] method.
     */
    ...
    /**
     * Internal rendering that needs to be done on the node that shouldn't be overridden. Calls
     * [propagateInternalRender] method.
     */
    fun propagateRender(
        batch: Batch,
        camera: Camera,
    ...
    /**
     * Internal post rendering that needs to be done on the node that shouldn't be overridden. Calls
     * [propagatePostRender] method.
     */
    ...
    /**
     * Invoked before [render]. Calculations or logic that needs to be done before rendering such as
     * flushing the batch or starting a frame buffer. The [Camera] can be used for culling and the
     * [Batch] instance to draw with.
     *
    ...
    /**
     * The main render method. The [Camera] can be used for culling and the [Batch] instance to draw
     * with.
     *
     * @param batch the batcher
     * @param camera the Camera2D node
    ...
    /**
     * Invoked after [render]. Calculations or logic that needs to be done after rendering such as
     * flushing the batch or ending a frame buffer. The [Camera] can be used for culling and the
     * [Batch] instance to draw with.
     *
     * @param batch the batcher
     * @param camera the Camera2D node
    ...
    /**
     * Sets the position of the [CanvasItem] relative to the parent [Node]. If the [CanvasItem] has
     * no parent or if the parent node is NOT a [CanvasItem], then it is the same a [globalPosition]
     *
     * @param value the new position
     * @return the current [CanvasItem]
     */
    ...
    fun globalRotation(angle: Angle): CanvasItem {
        ...
        (parent as? CanvasItem)?.let { rotation = it.globalRotation + angle }
            ?: run { rotation = angle }
    ...
    /**
     * Sets the rotation of the [CanvasItem] relative to the parent [Node] in radians. If the
     * [CanvasItem] has no parent or if the parent node is NOT a [CanvasItem], then it is the same a
     * [globalRotation]
     *
     * @param angle the new rotation
    ...
    /**
     * Sets the scale of the [CanvasItem] relative to the parent transform's scales. If the
     * [CanvasItem] has no parent or if the parent node is NOT a [CanvasItem], then it is the same a
     * [globalScale]
     */
    ...
    /**
     * Internal. Called when the hierarchy of this [CanvasItem] is changed. Example changes that can
     * trigger this includes: `position`, `rotation`, and `scale`
     */
    ...
    /**
     * Called when the hierarchy of this [CanvasItem] is changed. Example changes that can trigger
     * this include: `position`, `rotation`, and `scale`
     */
    ...
    /**
     * Transforms the provided local position into a position in global coordinate space. The input
     * is expected to be local relative to the Node2D it is called on. e.g. Applying this method to
     * the positions of child nodes will correctly transform their positions into the global
     * coordinate space, but applying it to a node's own position will give an incorrect result, as
     * it will incorporate the node's own transformation into its global position.
     */
     ...
     * it will incorporate the node's own transformation into its global position.
     */
    fun toGlobal(x: Float, y: Float, out: MutableVec2f): MutableVec2f =
        localToGlobalTransform.transform(out.set(x, y))
    ...
    fun toLocal(x: Float, y: Float, out: MutableVec2f): MutableVec2f =
        globalInverseTransform.transform(out.set(x, y))

    /**
     * Copy the properties from the given [CanvasItem] in this [CanvasItem].
     *
     * @param node the canvas item to copy the data from
     */
