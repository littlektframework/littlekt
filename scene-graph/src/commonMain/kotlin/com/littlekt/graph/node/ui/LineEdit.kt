 ...
 */
@OptIn(ExperimentalContracts::class)
inline fun SceneGraph<*>.lineEdit(
    callback: @SceneGraphDslMarker LineEdit.() -> Unit = {}
): LineEdit {
    contract { callsInPlace(callback, InvocationKind.EXACTLY_ONCE) }
...
open class LineEdit : Control() {
    ...
    private val availableWidth
        get() = width - bg.marginLeft - bg.marginRight
    ...
    /** The placeholder text that appears when [text] is empty. */
    var placeholderText: String = ""

    /** The [Char] to use for masking [text]. */
    var secretCharacter: Char = '*'

    /** Mark this field a secret and masks the [text] with [secretCharacter]. */
    ...
    /** The color of the font used to render the text. */
    var fontColor: Color
    ...
    /** The color of the font when the field is disabled. */
    ...
    /** The color of the font for displaying the [placeholderText]. */
    ...
    /** The disabled background [Drawable]. */
    ...
    /** The [Drawable] for when [text] is selected. */
    ...
    /** The [Drawable] for drawing the caret. */
    var caret: Drawable
    ...
    /** The [Drawable] for when the field [hasFocus]. */
    ...
    override fun uiInput(event: InputEvent<*>) {
        ...
        if (
            event.type == InputEvent.Type.KEY_DOWN ||
                event.type == InputEvent.Type.KEY_REPEAT ||
                event.type == InputEvent.Type.ACTION_DOWN ||
                event.type == InputEvent.Type.ACTION_REPEAT
        ) {
            val shift =
                scene?.context?.input?.isKeyPressed(Key.SHIFT_LEFT) == true ||
                    scene?.context?.input?.isKeyPressed(Key.SHIFT_RIGHT) == true
            val ctrl =
                scene?.context?.input?.isKeyPressed(Key.CTRL_LEFT) == true ||
                    scene?.context?.input?.isKeyPressed(Key.CTRL_RIGHT) == true

            // we gotta double check ui signals for uiLeft and uiRight since those are used for
            // focus. We need to override them.
            val uiLeft = scene?.uiInputSignals?.uiLeft
            val uiRight = scene?.uiInputSignals?.uiRight
            ...
            when (event.key) {
                ...
                Key.V -> {
                    if (ctrl && editable) {
                        ...
                        scene?.context?.clipboard?.contents?.let { insertAtCaret(it) }
                        event.handle()
                    }
                }
                Key.Z -> {
