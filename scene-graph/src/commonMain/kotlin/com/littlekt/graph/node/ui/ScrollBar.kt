...
/**
 * A abstract [Range]-based [Control] that displays a draggable area ([page]). See [HScrollBar] or
 * [VScrollBar] in order to use it.
 *
 * @author Colton Daily
 * @date 10/16/2022
 */
abstract class ScrollBar(val orientation: Orientation = Orientation.VERTICAL) : Range() {
    ...
    override fun uiInput(event: InputEvent<*>) {
        ...
        if (touchDown) {
            ...
            val decrementSize =
                if (orientation == Orientation.VERTICAL) decrement.minHeight else decrement.minWidth
            val incrementSize =
                if (orientation == Orientation.VERTICAL) increment.minHeight else increment.minWidth
            ...
            if (orientation == Orientation.VERTICAL) {
                if (offset < incrementSize) {
                    incrementActive = true
                    value += step
                    return
                }

                if (offset > total - decrementSize) {
        ...
        if (
            event.type == InputEvent.Type.MOUSE_ENTER || event.type == InputEvent.Type.MOUSE_HOVER
        ) {
    ...
    override fun render(batch: Batch, camera: Camera, shapeRenderer: ShapeRenderer) {
        ...
        if (orientation == Orientation.HORIZONTAL) {
            ...
            grabberY += getGrabberOffset() + decrement.minHeight + bg.marginBottom
        }
    ...
    private fun getGrabberOffset(): Float {
        if (orientation == Orientation.HORIZONTAL) return getAreaSize() * ratio
        return getAreaSize() * (1f - ratio) - getGrabberSize() + scroll.minHeight
    }
