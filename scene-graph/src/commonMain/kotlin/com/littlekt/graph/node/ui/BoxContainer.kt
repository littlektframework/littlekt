...
abstract class BoxContainer : Container() {
    ...
    /** The separation between children [Control] nodes. */
    var separation: Int
    ...
    /** The alignment of the children [Control] nodes. */
        ...
        nodes.forEach {
            if (it is Control && it.enabled && it.visible && !it.isDestroyed) {
                if (vertical) {
                    ...
                    _internalMinHeight +=
                        it.combinedMinHeight + if (first) 0f else separation.toFloat()
                } else {
                    if (it.combinedMinHeight > _internalMinHeight) {
                        _internalMinHeight = it.combinedMinHeight
                    }
                    _internalMinWidth +=
                        it.combinedMinWidth + if (first) 0f else separation.toFloat()
                }
                first = false
            }
        }
    ...
    override fun onSortChildren() {
            ...
            if (it is Control) {
                if (it.enabled && it.visible && !it.isDestroyed) {
                    ...
                    if (vertical) {
                        ...
                        willStretch = it.verticalSizing.isFlagSet(SizeFlag.EXPAND)
                    } else {
                        stretchMin += it.combinedMinWidth.toInt()
                        minSize = it.combinedMinWidth.toInt()
                        willStretch = it.horizontalSizing.isFlagSet(SizeFlag.EXPAND)
                    }
        ...
        val stretchMax =
            (if (vertical) height.toInt() else width.toInt()) - (childrenCount - 1) * separation
