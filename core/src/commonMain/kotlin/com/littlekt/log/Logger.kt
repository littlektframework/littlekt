...
class Logger private constructor(val name: String) {
    ...
    val isLocalLevelSet: Boolean
        get() = Logger_levels[name] != null

    /** Check if the [output] is set for this [Logger] */
    val isLocalOutputSet: Boolean
        get() = Logger_outputs[name] != null

    companion object {
        private val Logger_loggers: AtomicMap<String, Logger> = atomic(emptyMap())
        private val Logger_levels: AtomicMap<String, Level?> = atomic(emptyMap())
        private val Logger_outputs: AtomicMap<String, Output?> = atomic(emptyMap())
        ...
        operator fun invoke(name: String) = Logger_loggers[name] ?: Logger(name)

        /** Gets a [Logger] from its [KClass.simpleName] */
        inline operator fun <reified T : Any> invoke() =
            invoke(T::class.simpleName ?: "NoClassName")
    }
    ...
    object ConsoleLogOutput : Output {
        override fun output(logger: Logger, level: Level, msg: Any?) {
            when (level) {
                ...
                Level.TRACE -> Console.trace("[${logger.name}]", msg)
                Level.DEBUG -> Console.debug("[${logger.name}]", msg)
...
private typealias AtomicMap<K, V> = AtomicRef<Map<K, V>>
