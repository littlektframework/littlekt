...
interface AnsiEscape {
    ...
    fun StringBuilder.appendAnsiScape(
        code: Int,
        extra: String? = null,
        char: Char = 'm'
    ): StringBuilder {
    ...
    fun StringBuilder.appendFgColor(color: Color, bright: Boolean = false) =
        appendAnsiScape(30 + color.index, extra = if (bright) ";1" else null)

    fun StringBuilder.appendBgColor(color: Color, bright: Boolean = false) =
        appendAnsiScape(40 + color.index, extra = if (bright) ";1" else null)
    ...
    fun String.color(color: Color, bright: Boolean = false) = buildString {
        appendFgColor(color, bright).append(this@color).appendReset()
    }

    fun String.bgColor(color: Color, bright: Boolean = false) = buildString {
        appendBgColor(color, bright).append(this@bgColor).appendReset()
    }

    fun String.ansiEscape(code: Int) = buildString {
        appendAnsiScape(code).append(this@ansiEscape).appendReset()
    }
    ...
    val String.yellow
        get() = color(Color.YELLOW)

    val String.blue
        get() = color(Color.BLUE)

    val String.purple
        get() = color(Color.PURPLE)

    val String.cyan
        get() = color(Color.CYAN)

    val String.white
        get() = color(Color.WHITE)

    val String.bgBlack
        get() = bgColor(Color.BLACK)

    val String.bgRed
        get() = bgColor(Color.RED)

    val String.bgGreen
        get() = bgColor(Color.GREEN)

    val String.bgYellow
        get() = bgColor(Color.YELLOW)

    val String.bgBlue
        get() = bgColor(Color.BLUE)

    val String.bgPurple
        get() = bgColor(Color.PURPLE)

    val String.bgCyan
        get() = bgColor(Color.CYAN)

    val String.bgWhite
        get() = bgColor(Color.WHITE)
}
