...
/**
 * An index of pointers that begins at [index] 0 and increments by 1. This can refer to mouse clicks
 * or touches.
 *
 * @author Colton Daily
 * @date 11/6/2021
...
enum class Pointer(val index: Int) {
    ...
    companion object {
        val cache = entries.toTypedArray()

        /** The pointer that refers to the left mouse-button. Alias for [POINTER1]. */
        val MOUSE_LEFT = POINTER1
        /** The pointer that refers to the right mouse-button. Alias for [POINTER2]. */
        val MOUSE_RIGHT = POINTER2
        /** The pointer that refers to the middle mouse-button. Alias for [POINTER3]. */
...
enum class GameButton(val index: Int) {
    ...
    companion object {
        val BUTTONS = entries.toTypedArray()
        val MAX = 32
...
class GamepadInfo(
    ...
    operator fun get(stick: GameStick): Point =
        axesData[stick.index].apply {
            this.x = getX(stick)
            this.y = getY(stick)
        }

    fun getX(stick: GameStick) =
        when (stick) {
            GameStick.LEFT -> get(GameAxis.LX)
            GameStick.RIGHT -> get(GameAxis.RX)
        }

    fun getY(stick: GameStick) =
        when (stick) {
...
abstract class GamepadMapping {
    ...
    val buttonToIndex by lazy {
        buttonListOrder.mapIndexed { index, gameButton -> gameButton to index }.toMap()
    }
    val indexToButton by lazy {
        buttonListOrder.mapIndexed { index, gameButton -> index to gameButton }.toMap()
    }
    ...
    fun get(axis: GameAxis, info: GamepadInfo): Float =
        axisToIndex[axis]?.let { info.getRawAxis(it) } ?: 0f
    ...
    fun toString(info: GamepadInfo) =
        "$id(" + GameButton.values().joinToString(", ") { "${it.name}=${get(it, info)}" } + ")"
}
...
open class StandardGamepadMapping : GamepadMapping() {
    ...
    override val buttonListOrder: List<GameButton> =
        listOf(
            GameButton.BUTTON0,
            GameButton.BUTTON1,
            GameButton.BUTTON2,
    ...
    override val axisListOrder: List<GameAxis> =
        listOf(GameAxis.LX, GameAxis.LY, GameAxis.RX, GameAxis.RY)
}
...
interface Input {
    ...
    /**
     * A list of [Key] types to catch to prevent calling the current platforms default
     * functionality.
     *
