...
class InputCache {
    ...
    private val gamepadButtonsLastState = mutableMapOf<Int, MutableMap<GameButton, Float>>()
    ...
    val anyKeyPressed: Boolean
        get() = totalKeysPressed > 0

    val anyGamepadButtonPressed
        get() = totalGamepadButtonsPressed > 0

    val isTouching: Boolean
        get() = touches > 0

    val currentEventTime: Long
        get() = queueManager.currentEventTime
    ...
    fun onGamepadJoystickMoved(stick: GameStick, x: Float, y: Float, gamepad: GamepadInfo) {
        val pos =
            gamepadStickLastPosition
                .getOrPut(gamepad.index) { mutableMapOf() }
                .getOrPut(stick) { FloatArray(2) }
        pos[0] = x
        pos[1] = y
    ...
    fun updateGamepadButtons(gamepad: GamepadInfo) {
        gamepad.mapping.buttonListOrder.forEach {
            ...
            val lastState =
                gamepadButtonsLastState.getOrPut(gamepad.index) { mutableMapOf() }[it] ?: 0f
    ...
    fun reset() {
        ...
        if (anyGamepadButtonsJustPressed || anyGamepadButtonsJustReleased) {
            GameButton.BUTTONS.fastForEach { button ->
                gamepadButtonsJustPressed.values.forEach { it[button] = false }
                gamepadButtonsJustReleased.values.forEach { it[button] = false }
            }
        }
