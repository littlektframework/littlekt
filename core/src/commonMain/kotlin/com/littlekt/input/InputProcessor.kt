...
class InputProcessBuilder {
    ...
    fun onTouchDraggedHandle(
        action: (screenX: Float, screenY: Float, pointer: Pointer) -> Boolean
    ) {
        touchDragged += action
    }
    ...
    fun onGamepadButtonPressedHandle(
        action: (button: GameButton, pressure: Float, gamepad: Int) -> Boolean
    ) {
        gameButtonPressed += action
    }

    fun onGamepadButtonPressed(
        action: (button: GameButton, pressure: Float, gamepad: Int) -> Unit
    ) {
    ...
    fun onGamepadJoystickMovedHandle(
        action: (stick: GameStick, xAxis: Float, yAxis: Float, gamepad: Int) -> Boolean
    ) {
        gameJoystickMoved += action
    }

    fun onGamepadJoystickMoved(
        action: (stick: GameStick, xAxis: Float, yAxis: Float, gamepad: Int) -> Unit
    ) {
    ...
    fun onGamepadTriggerChangedHandle(
        action: (button: GameButton, pressure: Float, gamepad: Int) -> Boolean
    ) {
        gamepadTriggerChanged += action
    }

    fun onGamepadTriggerChanged(
        action: (button: GameButton, pressure: Float, gamepad: Int) -> Unit
    ) {
    ...
    fun build() =
        object : InputProcessor {
            ...
            override fun touchDragged(screenX: Float, screenY: Float, pointer: Pointer): Boolean {
                var handled = false
                touchDragged.fastForEach {
                    if (it.invoke(screenX, screenY, pointer)) handled = true
                }
                return handled
            }
            ...
            override fun gamepadButtonPressed(
                ...
                var handled = false
                gameButtonPressed.fastForEach {
                    if (it.invoke(button, pressure, gamepad)) handled = true
                }
                return handled
            }
            ...
            override fun gamepadJoystickMoved(
                ...
                var handled = false
                gameJoystickMoved.fastForEach {
                    if (it.invoke(stick, xAxis, yAxis, gamepad)) handled = true
                }
                return handled
            }
            ...
            override fun gamepadTriggerChanged(
                ...
                var handled = false
                gamepadTriggerChanged.fastForEach {
                    if (it.invoke(button, pressure, gamepad)) handled = true
                }
                return handled
            }
