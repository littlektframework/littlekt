...
import com.littlekt.util.epochMillis
...
/**
 * An [InputProcessor] that handles input events and emits gesture related events such as flinging,
 * panning, zooming, and pinching and sends them to a [GestureProcessor]. Based off of libGDX
 * GestureDetector.
 *
 * @author Colton Daily
 * @date 10/24/2022
...
class GestureController(
    ...
    constructor(
        ...
        longPressDuration,
        maxFlingDuration,
        GestureProcessorBuilder().apply { setup() }.build()
    )
    ...
    override fun touchDragged(screenX: Float, screenY: Float, pointer: Pointer): Boolean {
        ...
        if (pinching) {
            ...
            return processor.zoom(initialPos1.distance(initialPos2), pointer1.distance(pointer2)) ||
                result
        }
    ...
    override fun touchUp(screenX: Float, screenY: Float, pointer: Pointer): Boolean {
        ...
        if (inTapRectangle) {
            if (
                lastTapPointer != pointer ||
                    epochMillis() - lastTapTime > tapCountInterval.milliseconds ||
                    !isWithinTapRectangle(screenX, screenY, lastTapX, lastTapY)
            ) {
                tapCount = 0
            }
            tapCount++
        ...
        if (time - touchDownTime <= maxFlingDuration.seconds) {
            ...
            handled =
                processor.fling(tracker.velocityX * 1000f, tracker.velocityY * 1000f, pointer) ||
                    handled
        }
...
/**
 * Creates a new [GestureController] using a [GestureProcessorBuilder] and calls
 * [Input.addInputProcessor] when built.
 *
