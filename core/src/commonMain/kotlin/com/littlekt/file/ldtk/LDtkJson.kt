...
data class LDtkMapData(
     ...
     */
    @SerialName("minifyJson") val minifyJSON: Boolean,
     ...
     * JSON. after the "Multiple worlds" update, there will be a worlds array in root, each world
     * containing levels and layout settings. Basically, it's pretty much only about moving the
     * levels array to the worlds array, along with world layout related values (eg. worldGridWidth
     * etc).
     */
...
data class LDtkEntityDefinition(
    ...
    @SerialName("tilesetId") val tilesetID: Int? = null,
...
data class LDtkFieldDefinition(
    /**
     * Human readable value type (eg. `Int`, `Float`, `Point`, etc.). If the field is an array, this
...
data class LDtkLayerDefinition(
     ...
     *
     * **WARNING**: the list order is not related to actual IntGrid values! As user can re-order
     * IntGrid values freely, you may value "2" before value "1" in this list.
     */
     ...
     */
    @SerialName("type") val layerDefinitionType: LDtkLayerType,
...
data class LDtkEntityInstance(
    ...
    @SerialName("__worldX") val worldX: Int? = null,

    /** Y world coordinate in pixels. Only available in GridVania or Free world layouts. */
    @SerialName("__worldY") val worldY: Int? = null,
...
data class LDtkEntityTocData(
    ...
    @Deprecated(
        "This will be removed completely on version 1.7.0+.",
        ReplaceWith("instancesData", "com.")
    )
...
data class LDtkNeighbourLevelData(
    /**
     * A lowercase string tipping on the level location (`n`orth, `s`outh, `w`est, `e`ast). Since
     * 1.4.0, this value can also be `<` (neighbour depth is lower), `>` (neighbour depth is
     * greater) or `o` (levels overlap and share the same world depth). Since 1.5.3, this value can
     * also be `nw`, `ne`, `sw` or `se` for levels only touching corners.
     */
    val dir: String,
    ...
    val levelIid: String = "",
    @Deprecated(
        "Will be removed completely in LDtk '1.2.0+'",
        replaceWith = ReplaceWith("levelIid")
    )
    val levelUid: Int = 0,
)
...
enum class LDtkWorldLayout(val value: String) {
    ...
    companion object : KSerializer<LDtkWorldLayout> {
        override val descriptor: SerialDescriptor
            get() {
                return PrimitiveSerialDescriptor(
                    "com.littlekt.file.ldtk.WorldLayout",
                    PrimitiveKind.STRING
                )
            }
