...
val PathInfo.folder: String
    get() = fullPath.substring(0, fullPathNormalized.lastIndexOfOrNull('/') ?: 0)
...
val PathInfo.baseName: String
    get() = fullPathNormalized.substringAfterLast('/')

/** /path\to/file.ext -> /path\to */
val PathInfo.parent: PathInfo
    get() = PathInfo(folder)
...
fun PathInfo.baseNameWithCompoundExtension(ext: String): String =
    if (ext.isEmpty()) baseNameWithoutCompoundExtension
    else "$baseNameWithoutCompoundExtension.$ext"

/** /path\to/file.1.EXT -> EXT */
val PathInfo.extension: String
    get() = baseName.substringAfterLast('.', "")
...
val PathInfo.mimeTypeByExtension
    get() = MimeType.getByExtension(extensionLC)
...
val Path.fullPathNormalized: String
    get() = pathInfo.fullPathNormalized
val Path.folder: String
    get() = pathInfo.folder
val Path.folderWithSlash: String
    get() = pathInfo.folderWithSlash
val Path.baseName: String
    get() = pathInfo.baseName
val Path.fullPathWithoutExtension: String
    get() = pathInfo.fullPathWithoutExtension
...
val Path.fullNameWithoutExtension: String
    get() = pathInfo.fullNameWithoutExtension
val Path.baseNameWithoutExtension: String
    get() = pathInfo.baseNameWithoutExtension
val Path.fullNameWithoutCompoundExtension: String
    get() = pathInfo.fullNameWithoutCompoundExtension
val Path.baseNameWithoutCompoundExtension: String
    get() = pathInfo.baseNameWithoutCompoundExtension
...
fun Path.baseNameWithCompoundExtension(ext: String): String =
    pathInfo.baseNameWithCompoundExtension(ext)

val Path.extension: String
    get() = pathInfo.extension
val Path.extensionLC: String
    get() = pathInfo.extensionLC
val Path.compoundExtension: String
    get() = pathInfo.compoundExtension
val Path.compoundExtensionLC: String
    get() = pathInfo.compoundExtensionLC
val Path.mimeTypeByExtension: MimeType
    get() = pathInfo.mimeTypeByExtension
...
fun PathInfo.normalize(): String {
    ...
    return if (schemeIndex >= 0) {
        ...
        path.substring(0, schemeIndex + take) +
            path.substring(schemeIndex + take).pathInfo.normalize()
    } else {
...
fun PathInfo.combine(access: PathInfo): PathInfo {
    ...
    return (if (access.pathInfo.isAbsolute()) access.pathInfo.normalize()
        else "$base/$access".pathInfo.normalize())
        .pathInfo
}
