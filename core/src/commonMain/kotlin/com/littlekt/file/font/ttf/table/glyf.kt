...
internal class GlyfParser(
            ...
            if (glyph.endPointIndices.isNotEmpty()) {
                if (numOfCoordinates > 0) {
                    ...
                    for (i in 0 until numOfCoordinates) {
                        flag = flags[i]
                        glyph.points[i]
                            .apply { x = parseGlyphCoord(p, flag, px, 2, 16) }
                            .also { px = it.x }
                    }
                    ...
                    for (i in 0 until numOfCoordinates) {
                        flag = flags[i]
                        glyph.points[i]
                            .apply { y = parseGlyphCoord(p, flag, py, 4, 32) }
                            .also { py = it.y }
                    }
                }
            }
    ...
    private fun parseGlyphCoord(
        p: Parser,
        flag: Int,
        prevValue: Int,
        shortVectorBitMask: Int,
        sameBitMask: Int
    ): Int {
        var v: Int
            ...
            glyph.refs.forEach { ref ->
                ...
                if (glyphRef.points.isNotEmpty()) {
                    ...
                    if (matchedPoints == null) {
                        ...
                        check(
                            matchedPoints[0] <= glyph.points.size - 1 &&
                                matchedPoints[1] <= glyphRef.points.size - 1
                        ) {
    ...
    fun transformPoints(
        ...
        ref: MutableGlyphReference
    ): MutableList<MutablePoint> {
