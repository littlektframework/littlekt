...
@PublishedApi internal const val MAX_DEGREES = 360f

@PublishedApi internal const val MAX_RADIANS = PI2_F

@PublishedApi internal const val HALF_DEGREES = MAX_DEGREES / 2f

@PublishedApi internal const val HALF_RADIANS = MAX_RADIANS / 2f
...
value class Angle(val radians: Float) : Comparable<Angle> {
    ...
    companion object {
        ...
        inline fun between(x0: Float, y0: Float, x1: Float, y1: Float): Angle =
...
val Angle.tangent
    get() = tan(this)
val Angle.degrees
    get() = Angle.radiansToDegrees(radians)

val Angle.absoluteValue: Angle
    get() = Angle.fromRadians(radians.absoluteValue)
...
fun Angle.inBetweenInclusive(min: Angle, max: Angle): Boolean =
    inBetween(min, max, inclusive = true)

fun Angle.inBetweenExclusive(min: Angle, max: Angle): Boolean =
    inBetween(min, max, inclusive = false)
...
val Angle.normalized
    get() = Angle(radians umod MAX_RADIANS)
