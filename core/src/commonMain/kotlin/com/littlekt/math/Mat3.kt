...
open class Mat3 {
    ...
    fun rotate(axis: Vec3f, degrees: Float, result: Mat3) =
        rotate(axis.x, axis.y, axis.z, degrees, result)
    ...
    fun invert(result: Mat3): Boolean {
        ...
        for (i in 0..2) {
            det +=
                (this[i] *
                    (this[3 + (i + 1) % 3] * this[6 + (i + 2) % 3] -
                        this[3 + (i + 2) % 3] * this[6 + (i + 1) % 3]))
        }
