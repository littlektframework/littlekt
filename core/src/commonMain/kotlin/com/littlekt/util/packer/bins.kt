...
class MaxRectsBin(override val options: PackingOptions = PackingOptions()) : BaseBin() {
    ...
    override fun add(rect: BinRect): BinRect? {
        return place(rect)?.also { _rects += it }
    }
    ...
    override fun repack(): List<BinRect> {
        ...
        unpacked.forEach { _rects.remove(it) }
        return unpacked
    }

    override fun clone(): BaseBin {
        val bin = MaxRectsBin(options)
        rects.forEach { bin.add(it) }
        return bin
    }
    ...
    private fun place(rect: BinRect): BinRect? {
        ...
        val node: BinRect? =
            findNode(
                rect.width + options.paddingHorizontal,
                rect.height + options.paddingVertical,
                allowRotation
            )
        if (node != null) {
            updateBinSize(node)
    ...
    private fun splitNode(freeRect: BinRect, usedNode: BinRect): Boolean {
        ...
        // do horizontal split
        if (
            usedNode.y < freeRect.y + freeRect.height && usedNode.y + usedNode.height > freeRect.y
        ) {
    ...
    private fun updateBinSize(node: BinRect): Boolean {
        ...
        var tmpWidth =
            max(width, node.x + node.width - options.paddingHorizontal + options.edgeBorder)
        var tmpHeight =
            max(height, node.y + node.height - options.paddingVertical + options.edgeBorder)

        if (options.allowRotation) {
            // if rotated node, check if it is a better choice
            val rotWidth =
                max(width, node.x + node.height - options.paddingHorizontal + options.edgeBorder)
            val rotHeight =
                max(height, node.y + node.width - options.paddingVertical + options.edgeBorder)
        ...
        if (
            tmpWidth > maxWidth + options.paddingHorizontal ||
                tmpHeight > maxHeight + options.paddingVertical
        ) {
            return false
        }
    ...
    private fun expandFreeRects(tw: Int, th: Int) {
        freeRects.forEach {
            if (
                it.x + it.width >= min(width + options.paddingHorizontal - options.edgeBorder, tw)
            ) {
                it.width = tw - it.x - options.edgeBorder
            }
            if (
                it.y + it.height >= min(height + options.paddingVertical - options.edgeBorder, th)
            ) {
        ...
        _freeRects =
            freeRects
                .filter {
                    !(it.width <= 0 ||
                        it.height <= 0 ||
                        it.x < options.edgeBorder ||
                        it.y < options.edgeBorder)
                }
                .toMutableList()
        pruneFreeList()
    }
...
class OversizedElementBin(rect: BinRect) : BaseBin() {
    ...
    override val options: PackingOptions =
