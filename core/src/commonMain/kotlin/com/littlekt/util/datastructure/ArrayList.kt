...
class ShortArrayList(capacity: Int = 7, var shouldGrow: Boolean = true) : Collection<Short> {
    ...
    var data: ShortArray = ShortArray(capacity)
        private set
    ...
    @Suppress("ReplaceSizeZeroCheckWithIsEmpty") override fun isEmpty(): Boolean = this.size == 0
    ...
    fun insertAt(index: Int, value: ShortArray, start: Int = 0, end: Int = value.size) =
        this.apply {
            val count = end - start
            ensure(count)
    ...
    fun removeAt(index: Int, count: Int): Short {
        ...
        if (count > 0) {
            if (index < length - count)
                arraycopy(data, index + count, data, index, length - index - count)
            length -= count
        }
        return out
    }
    ...
    fun subList(fromIndex: Int, toIndex: Int): List<Short> =
        data.asList().subList(fromIndex, toIndex)
    ...
    override fun equals(other: Any?): Boolean {
        if (other is ShortArrayList)
            return size == other.size && data.contentEquals(other.data, 0, size)
        if (other is List<*>) return other == this
        return false
    }

    override fun toString(): String =
        StringBuilder(2 + 5 * size)
            .also { sb ->
                sb.append('[')
...
class IntArrayList(capacity: Int = 7, var shouldGrow: Boolean = true) : Collection<Int> {
    ...
    var data: IntArray = IntArray(capacity)
        private set
    ...
    fun insertAt(index: Int, value: IntArray, start: Int = 0, end: Int = value.size) =
        this.apply {
            val count = end - start
            ensure(count)
            if (isNotEmpty()) arraycopy(data, index, data, index + count, length - index)
...
class DoubleArrayList(capacity: Int = 7, var shouldGrow: Boolean = true) : Collection<Double> {
    ...
    var data: DoubleArray = DoubleArray(capacity) as DoubleArray
        private set
    ...
    fun insertAt(index: Int, value: DoubleArray, start: Int = 0, end: Int = value.size) =
        this.apply {
            val count = end - start
            ensure(count)
            if (isNotEmpty()) arraycopy(data, index, data, index + count, length - index)
...
class FloatArrayList(capacity: Int = 7, var shouldGrow: Boolean = true) : Collection<Float> {
    ...
    fun insertAt(index: Int, value: FloatArray, start: Int = 0, end: Int = value.size) =
        this.apply {
            val count = end - start
            ensure(count)
            if (isNotEmpty()) arraycopy(data, index, data, index + count, length - index)
