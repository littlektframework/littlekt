...
/**
 * A Tiled map with all a list of [TiledLayer].
 *
 * @author Colton Daily
 * @date 2/28/2022
 */
class TiledMap(
    ...
    override fun render(batch: Batch, camera: Camera, x: Float, y: Float, scale: Float) =
        render(batch, camera, x, y, scale, displayObjects = false)
     ...
     */
    fun updateCachedAnimationTiles(cache: SpriteCache) {
        layers.forEach { it.updateAnimTiles(cache) }
    }

    /**
     * Render each layer in this Tiled map.
     *
     * @param displayObjects render objects in object layer that are assigned a tile; false
     *   otherwise
     * @param shapeRenderer if [displayObjects] is true, used to render object primitives, if
     *   renderer provided.
     */
    fun render(
        batch: Batch,
        ...
        layers.forEach {
            it.render(batch, camera, x, y, scale, displayObjects = displayObjects, shapeRenderer)
        }
    }
