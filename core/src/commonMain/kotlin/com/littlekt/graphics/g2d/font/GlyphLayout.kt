...
class GlyphLayout {
     ...
     * @param width the width to use for alignment, line wrapping, and truncation. May be zero if
     ...
     * @param truncate if not null and the width of the glyphs exceed [width], the glyphs are
     *   truncated with the glyphs of the specified truncate string. Truncate should not be used
     *   with text that contains multiple lines. Wrap is ignored if truncate is not null.
     */
    fun setText(
        font: Font,
        text: CharSequence,
        ...
        var lastRun = false
        var y = font.metrics.lineHeight * scaleY
        var runStart = 0
                    ...
                    if (wrapOrTruncate && (newLine || lastRun)) {
                        currentRun?.let { cr ->
                            ...
                            while (i < glyphRun.advances.size) {
                                ...
                                if (
                                    (wrapIndex == 0 &&
                                        glyphRun.x == 0f) // requires at least one glyph per line
                                    || wrapIndex >= glyphRun.glyphs.size
                                ) { // wrap al least the glyph that didn't fit
                                    wrapIndex = i - 1
                                }
                                val newRun =
                                    wrap(font, scaleX, glyphRun, wrapIndex)
                                        .also { currentRun = it }
...
class GlyphRun {
    ...
    fun getGlyphsFrom(
        ...
        do {
            ...
            advances +=
                if (currGlyph == null) (-glyph.left * scaleX - font.metrics.padding.left)
                else
                    (currGlyph.xAdvance + font.getKerningAmount(scaleX, currGlyph.code, ch.code)) *
                        scaleX
            currGlyph = glyph
        } while (i < end)
    ...
    override fun toString(): String {
        return buildString {
            appendLine(
                glyphs.map {
                    "${it.code.toChar()}: code=${it.code}, advance=${it.xAdvance}, width=${it.width}"
                }
            )
            appendLine(advances)
            append("\"")
...
private fun GlyphMetrics.calcWidth(font: Font, scale: Float) =
    right * scale - font.metrics.padding.right
