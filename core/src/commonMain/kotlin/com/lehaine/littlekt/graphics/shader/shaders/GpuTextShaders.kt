...
class GpuTextFragmentShader : FragmentShaderModel() {
    ...
    override var source: String = """
        ...
        void fetchBezier(int coordIndex, out vec2 p[3]) {
            for (int i=0; i<3; i++) {
                vec4 pixel = getPixelByXY(vec2(v_bezierCoord.x + 2 + coordIndex*3 + i, v_bezierCoord.y));
        ...
        void main() {
            ...
            mat2 initrot = inverse(mat2(dFdx(v_normCoord) * kPixelWindowSize, dFdy(v_normCoord) * kPixelWindowSize));
...
class GpuWebGLTextFragmentShader  : FragmentShaderModel() {
    val uTexture get() = parameters[0] as ShaderParameter.UniformSample2D
    val uTexelSize get() = parameters[1] as ShaderParameter.UniformVec2
    ...
    override var source: String = """
        ...
        void fetchBezier(int coordIndex, out vec2 p[3]) {
            for (int i=0; i<3; i++) {
                vec4 pixel = getPixelByXY(vec2(v_bezierCoord.x + 2.0 + float(coordIndex)*3.0 + float(i), v_bezierCoord.y));
                p[i] = vec2(normalizedUshortFromVec2(pixel.xy), normalizedUshortFromVec2(pixel.zw)) - v_normCoord;
            }
        }
        ...
        void updateClosestCrossing(in vec2 porig[3], mat2 M, inout float closest, vec2 integerCell) {
            ...
            for (int i=0; i<2; i++) {
                ...
                if (t[i] > 0.0 && t[i] < 1.0) {
                    ...
                    //if (posx > 0.0 && posx < 1.0 && posx < abs(closest)) {
                    if (sameCell && abs(posx) < abs(closest)) {
                        float derivy = 0.0; //tangentAt(p[0].y, p[1].y, p[2].y, t[i]);
                        closest = (derivy < 0.0) ? -posx : posx;
                    }
                }
            }
