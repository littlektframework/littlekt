...
actual class CommandEncoder(val delegate: GPUCommandEncoder) : Releasable {
    ...
    actual fun beginComputePass(label: String?): ComputePassEncoder {
        return ComputePassEncoder(delegate.beginComputePass(GPUObjectBase().apply {this.label = label }))
    }
...
actual class RenderPassEncoder(
    ...
    actual fun setVertexBuffer(slot: Int, buffer: GPUBuffer, offset: Long, size: Long) {
        delegate.setVertexBuffer(slot.toDouble(), buffer.delegate, offset.toDouble(), size.toDouble())
    ...
    actual fun setIndexBuffer(
        ...
        delegate.setIndexBuffer(buffer.delegate, indexFormat.nativeVal, offset.toDouble(), size.toDouble())
        EngineStats.setBufferCalls += 1
    }

    actual fun setBindGroup(index: Int, bindGroup: BindGroup, dynamicOffsets: List<Long>) {
        delegate.setBindGroup(index, bindGroup.delegate, dynamicOffsets.map { it.toDouble().toJsNumber() }.toJsArray())
