...
class ImageProcessor(val config: TexturePackerConfig) {
    ...
    private fun trim(image: BufferedImage, name: String): ImageRectData? {
        ...
        return ImageRectData(
            ...
            offsetY = top,
            originalWidth =
                if (config.crop == TexturePackerConfig.CropType.NONE) image.width - extrude * 2
                else width - extrude * 2,
            originalHeight =
                if (config.crop == TexturePackerConfig.CropType.NONE) image.height - extrude * 2
                else height - extrude * 2,
            image = image,
            name = name,
            extrude = extrude
        )
    ...
    private fun extrude(image: BufferedImage): BufferedImage {
        val extrude = config.packingOptions.extrude
        val out =
            BufferedImage(image.width + extrude * 2, image.height + extrude * 2, image.type).apply {
