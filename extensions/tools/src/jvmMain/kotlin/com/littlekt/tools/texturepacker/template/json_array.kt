...
internal data class AtlasPage(
    ...
    @Serializable internal data class Rect(val x: Int, val y: Int, val w: Int, val h: Int)

    @Serializable internal data class Size(val w: Int, val h: Int)
    ...
    internal data class Meta(
        ...
        @SerialName("related_multi_packs") val relatedMultiPacks: List<String> = listOf(),
    )
}
...
internal fun List<ImageRectData>.toFrames(
    cropType: TexturePackerConfig.CropType
): List<AtlasPage.Frame> {
    ...
    forEach { data ->
        output +=
            AtlasPage.Frame(
                ...
                trimmed =
                    (cropType != TexturePackerConfig.CropType.FLUSH_POSITION &&
                        (data.offsetX != 0 || data.offsetY != 0)) ||
                        data.regionWidth != data.width - data.extrude * 2 ||
                        data.regionHeight != data.height - data.extrude * 2,
                spriteSourceSize =
                    AtlasPage.Rect(
                        if (cropType == TexturePackerConfig.CropType.FLUSH_POSITION) 0
                        else data.offsetX,
                        if (cropType == TexturePackerConfig.CropType.FLUSH_POSITION) 0
                        else data.offsetY,
                        data.regionWidth,
