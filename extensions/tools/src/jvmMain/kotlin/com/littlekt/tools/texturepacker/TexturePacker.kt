...
class TexturePacker(val config: TexturePackerConfig) {
    ...
    fun process() {
        files =
            File(config.inputDir)
                .walkTopDown()
                .onEnter { !config.ignoreDirs.contains(it.invariantSeparatorsPath) }
                .filter {
                    extensions.contains(it.extension) &&
                        !config.ignoreFiles.contains(it.invariantSeparatorsPath)
                }
    ...
    private fun writeImages(outputDir: File, bins: List<Bin>) {
        ...
        bins.forEachIndexed { index, bin ->
            ...
            val imageName =
                if (bins.size > 1) "${config.outputName}-$index.png" else "${config.outputName}.png"
            val jsonName =
                if (bins.size > 1) "${config.outputName}-$index.json"
                else "${config.outputName}.json"
            val relatedMultiPacks =
                if (bins.size > 1) {
            ...
            if (config.packingOptions.bleed) {
                canvas =
                    ColorBleedEffect().processImage(canvas, config.packingOptions.bleedIterations)
            }
            val page =
                createAtlasPage(
                    canvas,
                    imageName,
                    bin.rects as List<ImageRectData>,
                    relatedMultiPacks,
                    config.crop
                )
            ...
            FileOutputStream(jsonFile).use { it.write(json.toByteArray()) }
        }
    }
